...
Every PHI read (client view, lists, timeline, note view, exports, accounting of disclosures), every write, every sign-in success and failure (with reason), MFA events, permission denials, break-glass access, configuration changes, backups and restores, key rotations, device revocations and wipes, API-key use, recovery drills and audit exports. Each entry records time, user, action, entity, client id, source IP (the proxy-appended address when `TRUST_PROXY` is set), success and structured details. Details never contain PHI (CLAUDE.md, enforced in review; failed sign-ins for unknown usernames are truncated and hashed). Nor do they contain free text a person typed: the reason given for a break-glass access, a record's deletion or merge, a legal hold being placed or cleared, an episode being reopened or a court order vacated is stored encrypted on the record it concerns (`breakglass_events.reason_enc`, `clients.removed_reason_enc`, `clients.legal_hold_reason_enc` / `legal_hold_cleared_reason_enc`, `episodes.reopen_reason_enc`, `court_orders.vacated_reason_enc`), and the audit entry records only `reason_recorded: true` (and, for break-glass, the id of the queued event). Entries written before schema 36 may still carry the text; the chain is append-only, so they are not rewritten — they age out with the retention purge. Code: `server/audit.js` `log`, called from every route; the rule "every PHI read/write must call `audit.log`" is in CLAUDE.md.
