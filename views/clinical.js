...
export async function overviewCard(clientId, { refresh } = {}) {
  ...
  if (s.problems) parts.push(h('div', { 'data-overview-problems': '1' }, h('h3', {}, `Active problems (${s.problems.length})`),
  ...
  if (s.care_plan) parts.push(h('div', { 'data-overview-careplan': '1' }, h('h3', {}, 'Care plan'),
  ...
  if (s.outcomes) parts.push(h('div', { 'data-overview-outcomes': '1' }, h('h3', {}, 'Outcome measures'),
  ...
  return h('div', { class: 'card', style: { gridColumn: '1 / -1' }, 'data-clinical-card': '1' }, h('h2', {}, 'Clinical picture'), h('div', { class: 'grid cols-2' }, parts));
}
function changeText(o) {
  ...
  return better ? h('span', { class: 'flag ok' }, `${d > 0 ? '+' : ''}${d} (better)`) : flag(`${d > 0 ? '+' : ''}${d} (worse)`, true, 'score got worse');
}
...
export async function carePlanTab(clientId, { refresh, clientDisplay } = {}) {
  ...
  const goalCard = (g) => h('div', { class: 'card mt', 'data-goal': g.id },
    ...
    g.steps.length ? table([
      ...
      { label: 'By', render: s => flag(fmt.date(s.target_date), s.overdue, 'overdue') },
