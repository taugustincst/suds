...
export function openNoteForm(values, { clientId, clientDisplay, kind, onDone, prefill } = {}) {
  ...
  function renderStructured() {
    ...
    structuredBox.append(h('fieldset', {}, h('legend', {}, `${fmtSel.value} sections`), secs.map(([k, label]) => h('div', { class: 'field' }, h('label', {}, `${k} — ${label}`), h('textarea', { 'data-sec': k, rows: 3 }, vals[k] || '')))));
    structuredBox.querySelectorAll('textarea').forEach(t => t.addEventListener('input', () => { const s = readStructured(); autoText = Object.entries(s).filter(([, v]) => v).map(([k, v]) => `${k}: ${v}`).join('\n\n'); if (!contentArea.value || contentArea.dataset.auto === '1') { contentArea.value = autoText; contentArea.dataset.auto = '1'; } }));
...
export async function openNote(id, { onChange } = {}) {
  ...
  const body = h('div', {},
    h('div', { class: 'row mb' }, badge(n.kind === 'clinical' ? 'Clinical' : 'Administrative', n.kind === 'clinical' ? 'purple' : 'info'), badge(fmt.label(n.status), statusKind(n.status)), badge(n.format), n.source !== 'manual' ? badge(`Imported: ${fmt.label(n.source)}`, 'warn') : null, n.part2_protected ? badge('42 CFR Part 2', 'danger') : null),
    ...
    n.structured ? h('div', { class: 'mt' }, Object.entries(n.structured).map(([k, v]) => v ? h('div', { class: 'mb' }, h('b', {}, k), h('div', { style: { whiteSpace: 'pre-wrap' } }, v)) : null)) : h('pre', { class: 'note mt' }, n.content),
