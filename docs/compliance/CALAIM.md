...
| **Assessment** by domain — for DMC-ODS, the **ASAM Criteria** multidimensional assessment and level-of-care determination | Client → **Assessments → ASAM** (`asam_assessments`; `/api/clients/:id/asam`) | A 0–4 risk rating per dimension with the reasoning (encrypted), the level of care recommended, the level referred to, and the reason for any difference (required when they differ). The latest sets the client's *ASAM level* and appears on the Overview; history is kept. Only dimension names (ASAM 3rd edition, as DHCS cites) and ratings are stored — the copyrighted ASAM Criteria text is not reproduced; use your programme's licensed materials for the rating definitions. |
...
| | Problem list & care plan (`careplan:*`) | ASAM & outcome measures (`assessments:*`) | Outcomes report (aggregate) | Outcomes export |
|---|---|---|---|---|
...
| DAST-10 | 10, yes/no (question 3 reverse scored) | 0 none, 1–2 low, 3–5 moderate, 6–8 substantial, 9–10 severe | © Harvey A. Skinner; free for non-commercial clinical use with credit (the credit is shown on the form) |
