...
| **County IT gate: audit** | **In place.** Hash-chained audit, daily sealed head, incremental and weekly full verification, 7-year retention | [docs/HIPAA.md](../HIPAA.md), [docs/security/LOGGING-AND-AUDIT.md](../security/LOGGING-AND-AUDIT.md) | Separate the audit key from the index key (listed in the risk register) |
| **County IT gate: recovery** | **In place and drilled.** Scheduled encrypted off-host backups, UI restore, pre-migration snapshots, DR drill with RTO/RPO | [docs/DEPLOYMENT.md](../DEPLOYMENT.md), [docs/security/BACKUP-AND-DR.md](../security/BACKUP-AND-DR.md) | County runs its own restore drill during the pilot |
