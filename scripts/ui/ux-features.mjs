...
await page.click('.search-results a'); await settle(page);
ok(/#\/client\//.test(page.url()), 'clicking a search result opens that client', page.url());
// quick log → note with autosave
await page.click('.appbar .btn.quick'); await page.waitForSelector('.quick-list button:has-text("Note")'); await page.click('.quick-list button:has-text("Note")'); await page.waitForSelector('.modal textarea[name=content]');
...
await page.fill('.modal textarea[name=content]', 'Autosave test from desktop.');
// Autosave fires 2.5 s after typing stops (views/notes.js); wait until the status has moved past
// "Unsaved changes" / "Saving…" (the check below alone would accept "Unsaved", which contains "saved").
await page.waitForFunction(() => !/unsaved|saving|not saved/i.test(document.querySelector('.modal .autosave')?.textContent || ''), null, { timeout: 15000, polling: 100 }).catch(() => {});
...
// help tip
await page.goto(base + '/#/interventions'); await settle(page); await page.click('.help-btn'); await settle(page);
...
// theme pref synced
await page.click('text=Light/dark'); await settle(page);
...
if (picker) {
  ...
  await picker.fill(surname.slice(0, 3).toLowerCase()); await page.waitForSelector('.modal [role=combobox][aria-expanded=true]', { timeout: 5000 }).catch(() => {}); await settle(page);
