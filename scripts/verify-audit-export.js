...
//
//   npm run verify-audit-export -- <export.ndjson> [--key <64 hex> | --key-file <file>] [--anchors <dir>] [--json]
//
...
function parseArgs(argv) {
  const a = { file: null, key: null, anchors: null, json: false };
...
function main(argv) {
  ...
  if (!a.file) { console.error('Usage: npm run verify-audit-export -- <export.ndjson> [--key <hex> | --key-file <keys.json>] [--anchors <dir>] [--json]'); return 2; }
  ...
  const r = verifyExport(fs.readFileSync(a.file, 'utf8'), { key: a.key ? Buffer.from(a.key, 'hex') : null, extraAnchors });
