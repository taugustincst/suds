...
| 16 | De-identification? | Exports are de-identified to HIPAA Safe Harbor by default: dates reduced to the year, `90+` for ages over 89, ZIP3 (`000` for restricted areas), an allow-list of columns with no free text, coded fields held to their lists, and a random record id for each export instead of the client code. Identified exports require a recipient/purpose and are accounted. The funder report, naloxone log and settlement report suppress counts of people under 11, with complementary suppression, and are labelled for publication only for the whole programme and one standard period that has ended; other aggregate reports do not ([HIPAA.md](../HIPAA.md)). |
...
| # | Question | Answer |
| --- | --- | --- |
| 30 | Backups? | Encrypted, verified on write, retained N copies, copied offsite when an offsite directory is set; optional frequent snapshots (`backup_schedule_minutes`) bring RPO to minutes. Scheduling and the offsite copy must be configured by the county (production warns when they are not; the setup wizard defaults production to every 4 hours). [BACKUP-AND-DR.md](BACKUP-AND-DR.md) |
...
| 32 | High availability? | Single instance by design; documented warm-standby (active–passive) failover procedure — the recovery drill tests restore, not failover. RPO can be minutes with snapshots. No automatic failover. |
