...
function ndp(ctx, { from, to, ts, tsP }) {
  ...
  for (const x of db.all(`SELECT i.occurred_at, i.location, i.client_id IS NULL AS anon, i.naloxone_kits kits FROM interventions i LEFT JOIN clients c ON c.id=i.client_id
    ...
    const date = dayOf(x.occurred_at); const site = x.location || 'unknown';
...
function settlement(ctx, { from, to, ts, tsP }) {
  ...
  const services = db.all(`SELECT COALESCE(f.settlement_use,'uncategorised') AS use_code, COUNT(*) services, COUNT(DISTINCT i.client_id) people, COALESCE(SUM(i.naloxone_kits),0) naloxone_kits
