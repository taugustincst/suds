...
function seed({ actor, workers, clinician = null, supervisor, seedValue = 42 }) {
  ...
  db.transaction(() => {
    ...
    cids.forEach((c, i) => {
      ...
      for (let k = 0; k < nt; k++) { const [title, pri, dueOff] = TASKS[(i + k * 2) % TASKS.length]; const done = c.cstatus === 'closed' || rand() < 0.3; db.run(`INSERT INTO tasks(id,client_id,assigned_to,created_by,title_enc,description,due_at,priority,status,is_milestone,completed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`, track('tasks', uuid()), c.id, c.worker, k === 0 ? supervisor : c.worker, encrypt(title), null, d(-dueOff), pri, done ? 'done' : 'open', title.includes('intake') ? 1 : 0, done ? d(1) : null); }
      // Notes
