...
const datetimeOf = (v) => { if (v === null || v === undefined || v === '') return null; if (typeof v === 'number') { const d = excelDate(v); return d ? new Date(d + 'T12:00:00').toISOString() : undefined; } const d = new Date(String(v).trim()); if (!isNaN(d)) return d.toISOString(); const day = dateOf(v); return day ? new Date(day + 'T12:00:00').toISOString() : undefined; };
...
const ENTITIES = {
  ...
  resources: { label: 'Resource directory', table: 'resources', fields: [
    ...
    F('mat_offered', 'MAT offered', ['mat', 'moud'], str(200)), F('contact_person', 'Contact person', ['contact'], str(200)), F('notes', 'Notes', [], str(2000)),
  ] },
