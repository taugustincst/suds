...
* **How each result was established.** Every remark ends with *Evaluated by:* and the methods behind it,
  from the list under Evaluation Methods Used: **axe** (automated rules), **scripted** (the checks axe cannot
  make, in the same script), **keyboard** (keyboard-only task scripts), **review** (manual review of the source
  and the rendered pages). No criterion has yet been evaluated with a screen reader or speech recognition. Where
  a remark says something is "announced" or "exposed", that is what the markup does (a live region, an ARIA role
  or state, checked by axe and review) — not yet what a screen reader was heard to say. Criteria whose result
  depends on how assistive technology presents the page (4.1.2, 4.1.3) are therefore reported as Partially
  Supports until that testing is done.
...
| [3.1.1 Language of Page](https://www.w3.org/TR/WCAG21/#language-of-page) (Level A) | Supports | `lang="en"` on every page. *Evaluated by:* axe (`html-has-lang`, `html-lang-valid`). |
| [3.2.1 On Focus](https://www.w3.org/TR/WCAG21/#on-focus) (Level A) | Supports | Focus never changes context. Focusing the client search shows its list of matches below it (content, not context) and Escape closes it. *Evaluated by:* keyboard, review. |
| [3.2.2 On Input](https://www.w3.org/TR/WCAG21/#on-input) (Level A) | Supports | Changing a filter redraws the list on the same page (a change of content) and returns focus to the filter; no input submits a form or opens a new window by itself. *Evaluated by:* keyboard, review. |
| [3.3.1 Error Identification](https://www.w3.org/TR/WCAG21/#error-identification) (Level A) | Supports | A form with a problem shows a summary in an alert ("Fill in First name and Last name below."), writes the message under each field naming it ("First name is required"), sets `aria-invalid`, ties the message with `aria-describedby`, puts the summary in an alert region, and moves focus to the first field in error. Server-side errors are shown the same way. The error is identified in visible text, which is what 3.3.1 requires. *Evaluated by:* keyboard (a form error that is shown, named, tied to its field and focused), axe (`aria-valid-attr-value`), review; what a screen reader announces has not yet been checked. |
| [3.3.2 Labels or Instructions](https://www.w3.org/TR/WCAG21/#labels-or-instructions) (Level A) | Supports | Every field has a visible label; required fields are marked `*`; format help sits under the field (passwords, dates, client search). *Evaluated by:* axe (`label`, `select-name`), review of every form. |
| [4.1.1 Parsing](https://www.w3.org/TR/WCAG21/#parsing) (Level A) | Supports | The DOM is built programmatically (no hand-written markup errors) and axe finds no duplicate IDs or invalid nesting. (Per the WCAG 2.1 errata, 4.1.1 is always satisfied for HTML.) *Evaluated by:* axe (`duplicate-id-aria`, nesting rules). |
| [4.1.2 Name, Role, Value](https://www.w3.org/TR/WCAG21/#name-role-value) (Level A) | Partially Supports | Native controls are used throughout; custom widgets expose their role and state: the client search (`combobox`/`listbox`/`option`, `aria-expanded`, `aria-activedescendant`), the Log in / Sign up tabs (`tablist`/`tab`/`tabpanel`, `aria-selected`), section navigation (`aria-current="page"`), the "More" and help buttons (`aria-expanded`, `aria-controls`), dialogs (`role="dialog"`, `aria-modal`, named by their title), status messages. No interactive control is nested inside another. *Evaluated by:* axe (every name, role, ARIA-attribute and nesting rule, on every page and dialog, with no findings), keyboard (the combobox, tabs and dialogs operated), review of each custom widget against the WAI-ARIA Authoring Practices. **Not yet established:** that screen readers (NVDA, JAWS, VoiceOver, TalkBack) actually present these names, roles, states and changes to them as intended — axe checks that the attributes are present and valid, not how assistive technology reads them. No defect is known; the result stays Partially Supports until that testing is done (see the remediation plan). |
...
| [1.3.4 Orientation](https://www.w3.org/TR/WCAG21/#orientation) (Level AA 2.1 only) | Supports | Works in portrait and landscape; the web app manifest sets `"orientation": "any"`. *Evaluated by:* review; scripted (phone and desktop viewports). |
| [1.3.5 Identify Input Purpose](https://www.w3.org/TR/WCAG21/#identify-input-purpose) (Level AA 2.1 only) | Supports | Fields about the person using SUDS carry `autocomplete` (`username`, `current-password`, `new-password`, `name`, `email`, `one-time-code`). Fields about clients describe someone else and are deliberately `autocomplete="off"`. *Evaluated by:* axe (`autocomplete-valid`), review. |
| [1.4.3 Contrast (Minimum)](https://www.w3.org/TR/WCAG21/#contrast-minimum) (Level AA) | Supports | Text is at least 4.5:1 (large text 3:1) in the light and the dark theme, tested on every page and dialog. Dark-theme filled buttons and badges use dark text; banners take their colours from theme tokens; links inside banners take the banner's text colour. *Evaluated by:* axe (`color-contrast`) in both themes; review of the colour tokens. |
| [1.4.4 Resize text](https://www.w3.org/TR/WCAG21/#resize-text) (Level AA) | Supports | All sizes are in `rem`, the sidebar width included; tested at 200% text on every page with no clipped or overlapping content and no sideways scrolling (data tables scroll within their frame). *Evaluated by:* scripted (200% text on every page). |
| [1.4.5 Images of Text](https://www.w3.org/TR/WCAG21/#images-of-text) (Level AA) | Supports | No images of text; the logo is decorative. *Evaluated by:* review. |
| [1.4.10 Reflow](https://www.w3.org/TR/WCAG21/#reflow) (Level AA 2.1 only) | Supports | Tested at 320 CSS px on every page and dialog: card grids fall to one column, tables become one card per record on phones, long words and badges wrap. Wide data tables (reports, budget lines) scroll inside their own focusable frame — two-dimensional content, which 1.4.10 exempts. *Evaluated by:* scripted (320 CSS px on every page and dialog). |
| [1.4.11 Non-text Contrast](https://www.w3.org/TR/WCAG21/#non-text-contrast) (Level AA 2.1 only) | Supports | Text-field and select borders are at least 3:1 against the page and cards (#7d8a99 light, 3.25:1 or more; #66788c dark, 3.42:1 or more); the focus ring is 3:1 or more in both themes; the current section's underline is the primary colour (6.8:1). Chart bars are paired with their values as text. *Evaluated by:* review (contrast measured from the colour tokens in both themes); scripted (visible focus at every stop). |
| [1.4.12 Text Spacing](https://www.w3.org/TR/WCAG21/#text-spacing) (Level AA 2.1 only) | Supports | Tested with line height 1.5, paragraph spacing 2, letter spacing 0.12 and word spacing 0.16 on every page: nothing is clipped. A resource card's summary is shortened to three lines by design; the full text is on the resource's page, one activation away. *Evaluated by:* scripted (the 1.4.12 overrides on every page). |
| [1.4.13 Content on Hover or Focus](https://www.w3.org/TR/WCAG21/#content-on-hover-or-focus) (Level AA 2.1 only) | Supports | The only content shown on focus is the client search's match list, which is in the page flow (covers nothing), stays until focus leaves, and closes with Escape. Help ("?") text opens on activation, not hover, and closes with Escape or the same button. Native `title` tooltips are browser-controlled. *Evaluated by:* keyboard, review. |
| [2.4.5 Multiple Ways](https://www.w3.org/TR/WCAG21/#multiple-ways) (Level AA) | Supports | Navigation menu, global client search, links from Home and between related records, and the reminders bell. *Evaluated by:* review. |
| [2.4.6 Headings and Labels](https://www.w3.org/TR/WCAG21/#headings-and-labels) (Level AA) | Supports | Headings name each page, card and dialog; labels describe each field. *Evaluated by:* axe (`page-has-heading-one`, `empty-heading`, `heading-order`, `aria-dialog-name`, `label`) for presence and structure, and review of the wording of every heading and label for whether it describes its topic or purpose — which no automated tool can judge. |
| [2.4.7 Focus Visible](https://www.w3.org/TR/WCAG21/#focus-visible) (Level AA) | Supports | A 2–3 px focus ring (`--focus`, 3:1 or more in both themes) on every control, including form fields (which previously showed only a faint ring), table rows, scrolling regions and the file button; checked by tabbing through every page. Date fields' internal calendar button uses the browser's own ring. *Evaluated by:* scripted (visible focus indicator at every Tab stop on every page). |
| [3.1.2 Language of Parts](https://www.w3.org/TR/WCAG21/#language-of-parts) (Level AA) | Supports | The interface is in English only. Text staff enter in another language is their content. *Evaluated by:* review. |
| [3.2.3 Consistent Navigation](https://www.w3.org/TR/WCAG21/#consistent-navigation) (Level AA) | Supports | The same sidebar (a menu on phones), search, reminders bell and + Log appear in the same order on every page; items a role may not open are left out, never reordered. *Evaluated by:* review; scripted (every page, each role). |
| [3.2.4 Consistent Identification](https://www.w3.org/TR/WCAG21/#consistent-identification) (Level AA) | Supports | Shared helpers give the same function the same name and look everywhere (+ Log, Close ✕, Edit, the calendar button, Load more). *Evaluated by:* review. |
| [3.3.3 Error Suggestion](https://www.w3.org/TR/WCAG21/#error-suggestion) (Level AA) | Supports | Messages say how to fix the problem: which field, the valid date range ("the year must be four digits, between 1900 and 2100"), the password rule, the reason length, how to search for a client, what a Part 2 consent must contain; a CalOMS record lists each edit-check problem by field and says whether it is fatal. *Evaluated by:* review of the validation messages; keyboard (form error task). |
| [3.3.4 Error Prevention (Legal, Financial, Data)](https://www.w3.org/TR/WCAG21/#error-prevention-legal-financial-data) (Level AA) | Supports | Signing a note (a legal attestation) needs the password and states that signed notes cannot be changed; deleting, revoking, erasing a device and restoring a backup ask for confirmation (a typed word for the irreversible ones); client records are removed only by the retention purge, and changes to them are recorded in the audit log; a record changed by someone else since it was opened is not overwritten silently. Disclosures are checked before they happen: a Part 2 consent must have every §2.31 element, a disclosure needs its lawful basis (a court order is checked for the findings subpart E requires), an agreed restriction on the record is shown for confirmation, and the CalOMS extract, county EHR hand-off and identified export each state that they identify clients and ask for confirmation. *Evaluated by:* review; keyboard (signing a note). |
| [4.1.3 Status Messages](https://www.w3.org/TR/WCAG21/#status-messages) (Level AA 2.1 only) | Partially Supports | Saved/done messages are in a polite live region (`role="status"`), errors in `role="alert"`; search results ("3 matching clients"), autosave state, a screening's running score while it is filled in, and the idle warning are written to live regions without moving focus. *Evaluated by:* review of where each status message is written (a live region already in the page when the message is inserted), axe (ARIA role and attribute rules), keyboard (the form-error and idle-warning scripts confirm the messages appear without taking focus). **Not yet established:** that screen readers announce them — live-region behaviour differs between screen readers and browsers and can only be confirmed by listening. No defect is known; the result stays Partially Supports until screen-reader testing is done. |
...
| 4.1.2, 4.1.3 | Evidence is automated (axe) and scripted keyboard testing only; no screen-reader testing yet, so whether names, roles, states and status messages reach assistive technology as intended is not established. | Test the scope above with NVDA with Firefox and Chrome, JAWS with Chrome, VoiceOver on macOS (Safari) and iOS, and TalkBack on Android, following the keyboard task list; record the results here, fix what is found, and move the criteria to Supports only on that evidence. The same testing reviews 1.1.1, 1.3.1, 1.3.2 and 3.3.1, and speech-recognition testing reviews 2.5.3, whose remarks note it is pending. |
