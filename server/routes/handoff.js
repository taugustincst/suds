...
// file — and one accounting-of-disclosures row per client it contains (server/disclosure.js).
const db = require('../db');
...
const CONSENT_TYPES = ['part2_disclosure', 'roi'];
const COLUMNS = [
...
/** The newest live consent to disclose (a Part 2 consent or a release of information) for a client, if any. */
function consentFor(clientId) {
  return db.one(`SELECT id FROM consents WHERE client_id=? AND type IN (${CONSENT_TYPES.map(() => '?').join(',')}) AND revoked_at IS NULL AND (expires_at IS NULL OR expires_at >= date('now')) ORDER BY signed_at DESC LIMIT 1`, clientId, ...CONSENT_TYPES) || null;
}
...
module.exports = (r) => {
  ...
  r.get('/api/handoff/export', auth.requireAuth, auth.requirePerm('export:identified'), async (ctx) => {
    ...
    // supervisor/administrator override and needs its written justification (disclosure.requireBasis).
