...
rows=()
failed=()
for s in ${SCRIPTS:-desktop review-fixes navigator-flow navigator-fixes ux-features local-mode multitab sync-two-way device-audit device-encryption spreadsheets sample-data resource-profiles forms region dates setup setup-same-origin static-site qa-retest clinical-audit ux-polish signup load-review a11y-round4 caloms frontline funder-reporting programme accessibility}; do
  echo "=== $s"
  ...
  timeout --kill-after=10 "${SUDS_SCRIPT_TIMEOUT:-900}" node scripts/ui/$s.mjs > $T/suds-ui-$s.log 2>&1; rc=$?
  [ $rc = 124 ] && echo "TIMED OUT after ${SUDS_SCRIPT_TIMEOUT:-900} s (a wait that never resolved)" >> $T/suds-ui-$s.log
  if [ $rc = 0 ]; then result=pass; grep -v '^\[2m' $T/suds-ui-$s.log | tail -6; else result=FAIL; echo "FAILED"; grep -v '^\[2m' $T/suds-ui-$s.log | tail -25; grep -E "^ *FAIL " $T/suds-ui-$s.log | head -10; fail=1; failed+=("$s"); fi
