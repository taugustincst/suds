...
// A public-looking name that resolves to the office's own network is refused before anything is fetched.
test('a name that resolves to a private or loopback address is refused, at every hop', async () => {
  const pictures = require('../server/region-pictures');
  let fetched = 0;
  pictures._setFetchForTests(async () => { fetched++; throw new Error('should not fetch'); });
  const answers = { 'intranet.example.org': ['10.1.2.3'], 'loop.example.org': ['127.0.0.1'], 'v6.example.org': ['fd00::5'], 'mapped.example.org': ['::ffff:192.168.1.9'], 'meta.example.org': ['169.254.169.254'] };
  pictures._setLookupForTests(async (h) => (answers[h] || ['93.184.216.34']).map(address => ({ address })));
  try {
    for (const host of Object.keys(answers)) {
      const r = await pictures.downloadFromAddress(`https://${host}/logo.png`);
      assert.equal(r.ok, false, host); assert.match(r.error, /not on the public internet/, host);
    }
    assert.equal(fetched, 0, 'nothing was fetched from any of them');
    // A redirect from a public name onto one of them is refused too.
    pictures._setFetchForTests(async (u) => { fetched++; return { ok: false, status: 302, headers: { get: (k) => (k === 'location' ? 'https://intranet.example.org/x.png' : null) }, arrayBuffer: async () => new ArrayBuffer(0) }; });
    const r = await pictures.downloadFromAddress('https://public.example.org/logo.png');
    assert.equal(r.ok, false); assert.match(r.error, /not on the public internet/);
    assert.equal(fetched, 1, 'only the public hop was fetched');
  } finally { pictures._setFetchForTests(null); pictures._setLookupForTests(null); }
  assert.equal(pictures.isPrivateAddress('8.8.8.8'), false);
  assert.equal(pictures.isPrivateAddress('2606:4700::1111'), false);
});
