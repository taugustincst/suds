...
route('dashboard', async () => {
  ...
  return h('div', {},
    ...
    h('div', { class: 'grid cols-4 mb' },
      ...
      stat('Calls', fmt.num(d.calls.total), '', 'calls'), stat('Open referrals', fmt.num(d.referrals.open), d.referrals.open ? 'warn' : '', 'referrals?status=open'), d.time ? stat(can('time:all') ? 'Team hours logged' : 'My hours logged', (d.time.minutes / 60).toFixed(1), '', 'time') : null, d.budget ? stat('Spent of budget', `${fmt.money(d.budget.spent)} / ${fmt.money(d.budget.total)}`, '', 'budget') : null),
