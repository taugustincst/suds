...
route('dashboard', async () => {
  ...
  return h('div', {},
    ...
    h('div', { class: 'grid cols-4 mb' },
      stat('Active clients', fmt.num(c.active)), stat('High-risk clients', fmt.num(c.high_risk), c.high_risk ? 'danger' : ''), stat('Visits & services (90 days)', fmt.num(i.total)), stat('Naloxone kits given', fmt.num(i.naloxone_kits)),
      stat('Calls', fmt.num(d.calls.total)), stat('Open referrals', fmt.num(d.referrals.open), d.referrals.open ? 'warn' : ''), d.time ? stat('My hours logged', (d.time.minutes / 60).toFixed(1)) : null, d.budget ? stat('Spent of budget', `${fmt.money(d.budget.spent)} / ${fmt.money(d.budget.total)}`) : null),
    h('div', { class: 'grid cols-2' },
      ...
      h('div', { class: 'card' }, h('h3', {}, 'What you have been doing (90 days)'), bars(i.by_type.slice(0, 8)), h('div', { class: 'mt' }, sparkline(i.by_week.map(w => w.n)), h('div', { class: 'muted small' }, 'visits per week'))),
      state.user.role === 'admin' ? h('div', { class: 'card' }, h('h3', {}, 'Use SUDS on phones'), h('p', { class: 'small muted' }, 'Staff can open SUDS on a phone or tablet on the office Wi-Fi and add it to their home screen. Show them the QR code under Settings → Network & devices.'), h('a', { class: 'btn sm', href: '#/admin?tab=network' }, 'Connect a phone')) : null,
