...
* The client page has tabs: Overview, Timeline (everything in date order), Interventions, Calls, Notes, Problems, Care plan, Assessments (clinicians and supervisors), Referrals, Forms, Tasks, Episodes, Consents & ROI, Requests, Time, Assistance $ (for roles that see the budget) and Care team. See *Problem list, care plan and assessments* below.
...
These follow the CalAIM Behavioral Health Documentation Redesign: a current **problem list**, **progress notes** that say which problems they address, a **care coordination plan**, and assessment by **ASAM dimension**. How each requirement maps to SUDS, and what is out of scope, is in `docs/compliance/CALAIM.md`.

* **Overview → Clinical picture** sums it up: active problems, how many care plan goals are active and any review that is overdue (in red), the latest ASAM assessment (the six ratings and the level of care), and each outcome measure's latest score with its trend. A PHQ-9 whose question 9 was answered above "Not at all" shows a red safety banner at the top. Each part appears only for roles that may see it.
...
* **Assessments** (clinicians and supervisors) — **+ ASAM assessment** records a 0–4 risk rating for each of the six ASAM dimensions with what supports it, the **recommended** level of care, the level **referred to**, and — required when they differ — why (waitlist, client preference, not available…). SUDS records the ratings and the decision; use your programme's ASAM Criteria materials for the rating definitions (SUDS does not reproduce them). The latest assessment sets the client's *ASAM level* field and is shown on the Overview; earlier ones stay in the history.
* **Outcome measures** — **+ PHQ-9**, **+ GAD-7**, **+ AUDIT-C**, **+ DAST-10** and **+ Wellbeing (0–10)** open the questionnaire. The score and its band appear as soon as every question is answered and are saved with the answers (encrypted). For AUDIT-C, choose the cut-off (men 4+, women 3+; unspecified uses 3+). Give the same measure again over time: the tab and the Overview show the trend. Click a result to see the answers; **Delete** is for a questionnaire entered in error.
