...
module.exports = (r) => {
  ...
  r.post('/api/clients/:id/episodes', auth.requireAuth, auth.requirePerm('episodes:write'), (ctx) => {
    ...
    db.transaction(() => {
      ...
      db.run(`UPDATE clients SET status=CASE WHEN status IN ('closed','inactive') THEN 'active' ELSE status END, discharge_date=NULL, discharge_reason=NULL, updated_at=? WHERE id=?`, db.now(), ctx.params.id);
    });
