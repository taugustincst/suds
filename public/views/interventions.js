...
export function openInterventionForm(values, { clientId, clientDisplay, onDone } = {}) {
  ...
  const f = form([
    ...
    { name: 'type', label: 'Intervention type', type: 'select', options: C.INTERVENTION_TYPES, required: true },
    ...
    { name: 'summary', label: 'Brief summary (non-clinical; use Notes for clinical documentation)', type: 'textarea', span: true, rows: 3 },
    { name: 'follow_up_due', label: 'Follow-up due (creates a task)', type: 'date' },
  ...
  ].filter(Boolean), { values: values || {}, submitText: isNew ? 'Log intervention' : 'Save', onCancel: () => m.close(), onSubmit: async (d) => {
  ...
  const m = modal(isNew ? 'Log intervention' : 'Edit intervention', f, { wide: true });
}
...
export function interventionTable(rows, { showClient = true, onChange } = {}) {
  return table([
    ...
    { label: '', render: r => (r.user_id === state.user.id || can('clients:all')) && can('interventions:write') ? h('div', { class: 'row nowrap' }, h('button', { class: 'btn sm', onClick: (e) => { e.stopPropagation(); openInterventionForm(r, { onDone: onChange }); } }, 'Edit'), h('button', { class: 'btn sm ghost', onClick: async (e) => { e.stopPropagation(); if (await confirmDialog('Delete intervention', 'Delete this intervention record? This is logged.', { danger: true, okText: 'Delete' })) { await del(`/api/interventions/${r.id}`); toast('Deleted'); onChange && onChange(); } } }, '✕')) : null },
  ].filter(Boolean), rows, { empty: 'No interventions recorded.' });
}
...
route('interventions', async (r) => {
  ...
  return h('div', {},
    pageHead('Interventions', can('interventions:write') ? h('button', { class: 'btn primary', onClick: () => openInterventionForm(null, { onDone: refresh }) }, '+ Log intervention') : null, h('button', { class: 'btn', onClick: () => downloadCsv(`/api/reports/export/interventions?from=${from || '2000-01-01'}&to=${to || fmt.today()}`) }, 'Export CSV')),
