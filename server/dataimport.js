...
const enumOf = (list) => (v) => { if (v === null || v === undefined || v === '') return null; if (typeof list === 'string') { const O = require('./options'); const es = O.entries(list).filter(e => !e.hidden); const t = String(v).trim().toLowerCase(); const byLabel = es.find(e => e.label.toLowerCase() === t); if (byLabel) return byLabel.code; return enumOf(es.map(e => e.code))(v); } const k = String(v).trim().toLowerCase().replace(/[\s-]+/g, '_'); let hit = list.find(x => x.toLowerCase() === k) || list.find(x => x.toLowerCase().replace(/_/g, '') === k.replace(/_/g, '')); if (hit === undefined) { const c = list.filter(x => x.includes(k) || k.includes(x)); if (c.length === 1) hit = c[0]; } return hit === undefined ? undefined : hit; };
...
const ENTITIES = {
  ...
  interventions: { label: 'Visits', table: 'interventions', fields: [
    clientRef, F('occurred_at', 'Date', ['date of service', 'service date', 'when', 'occurred'], datetimeOf, { required: true }), F('type', 'Type', ['service', 'intervention', 'service type', 'intervention type'], enumOf('INTERVENTION_TYPES'), { required: true, help: C.INTERVENTION_TYPES.join(', ') }),
    F('duration_minutes', 'Minutes', ['duration', 'duration minutes', 'time'], num), F('location', 'Location', ['where', 'setting'], enumOf('LOCATIONS')), F('modality', 'Modality', ['mode'], enumOf('MODALITIES')), F('outcome', 'Outcome', ['result'], enumOf('OUTCOMES')),
  ...
  calls: { label: 'Calls', table: 'calls', fields: [
    ...
    F('contact_type', 'Who', ['contact type', 'with', 'caller'], enumOf('CALL_CONTACT_TYPES')), F('contact_name', 'Contact name', ['contact'], str(120)), F('phone', 'Phone', ['number'], str(40)), F('duration_minutes', 'Minutes', ['duration', 'length'], num), F('outcome', 'Outcome', ['result'], enumOf('CALL_OUTCOMES')),
  ...
  time_entries: { label: 'Time', table: 'time_entries', fields: [
    F('work_date', 'Date', ['work date', 'day'], dateOf, { required: true }), F('minutes', 'Minutes', ['duration', 'time', 'mins'], num, { required: true }), F('category', 'Category', ['activity', 'type'], enumOf('TIME_CATEGORIES')), F('client_ref', 'Client', ['client code', 'client', 'code', 'client name'], str(120)),
