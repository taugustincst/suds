...
test('treatment center profile: summary, tags, pictures with validation and ordering', async () => {
  ...
  assert.equal(p1.status, 201, JSON.stringify(p1.data)); assert.equal(p1.data.photo.content_type, 'image/png'); assert.ok(p1.data.photo.thumb_url.startsWith('data:image/png;base64,'));
  ...
  assert.equal(row.photos.length, 2); assert.equal(row.photos[0].caption, 'Entrance'); assert.ok(row.photos[0].data_url.startsWith('data:image/png;base64,'));
  // directory shows the cover thumbnail and photo count
  const list = (await ro.get('/api/resources')).data.rows.find(r => r.id === id);
  assert.equal(list.photo_count, 2); assert.ok(list.cover_url.startsWith('data:image/png'));
