...
trap 'kill $SERVER $SETUP_SERVER $STATIC_SERVER 2>/dev/null; pkill -f "suds-setup-data" 2>/dev/null' EXIT
...
start_office() {
  ...
  echo "the office server did not start (see /tmp/suds-ui-server.log)" >&2; return 1
}
start_office || exit 2
...
rows=()
for s in ${SCRIPTS:-desktop review-fixes navigator-flow navigator-fixes ux-features local-mode multitab sync-two-way device-audit spreadsheets sample-data resource-profiles forms region dates setup static-site qa-retest clinical-audit ux-polish}; do
  echo "=== $s"
  ...
  t0=$SECONDS
  if node scripts/ui/$s.mjs > /tmp/suds-ui-$s.log 2>&1; then result=pass; grep -v '^\[2m' /tmp/suds-ui-$s.log | tail -6; else result=FAIL; echo "FAILED"; grep -v '^\[2m' /tmp/suds-ui-$s.log | tail -25; fail=1; fi
  # "name: 12/12 checks passed" is what makeChecks().finish() prints (scripts/ui/assert.mjs).
  checks=$(grep -oE '[0-9]+/[0-9]+ checks passed' /tmp/suds-ui-$s.log | tail -1 | cut -d' ' -f1)
