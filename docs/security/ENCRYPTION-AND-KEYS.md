...
**Device backups.** A backup (`local/backup.js`) holds the whole database and both column keys, sealed with AES-256-GCM under a key derived from a passphrase the person chooses (PBKDF2-SHA-256, 600,000 iterations), separate from any account password. It also carries each account's wrap, a fresh key for the device it will be restored onto, and that key sealed under this device's DEK — never this device's DEK. After a restore, the accounts in the backup sign in with the passwords they had when it was made and are re-wrapped at that sign-in. **The restored device does not stay on the key the backup carried.** That key is written in the backup, so anyone holding the file and its passphrase who later got the device's browser storage could otherwise read everything recorded after the restore (up to 1.12.0). The first sign-in after a restore by an account from the backup (or one vouching for someone) generates a fresh key, re-seals the database and the column keys under it and stores them with the new vault in one write (audit `device.key_rotated`). The signing account gets its own wrap of the new key; the other accounts from the backup keep theirs, which open the *backed-up* device's key with their passwords, and the chain is re-sealed from that key to the new one — the backed-up device's key was never in the backup (only password-protected wraps of it), so the backup's holder cannot follow it. Each is re-wrapped directly at its first sign-in, and the chain goes with the last. An account enrolled on the restored device before that first sign-in (vouched for, say) holds a wrap of the backup's key; that wrap is dropped at the rotation and the account is vouched for once more. **The remaining window:** from the restore to that first sign-in the device runs under the backup's key — nothing is recorded in it but the restore's own audit entry, unless someone without a backed-up account is let in first — and a browser's storage engine may keep superseded copies of the old sealed image on disk until it compacts them. The column keys themselves are not changed (the backup holds them too), which does not matter while the image is sealed under a key the backup's holder does not have; rotate them with a new device set-up if the backup and its passphrase were lost. The backup file and its passphrase are the only way back from a forgotten password with no other account on the device; the backup dialog says so.
