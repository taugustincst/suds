...
route('budget', async (r) => {
  ...
  const fundCard = (f) => h('div', { class: 'card' },
    h('div', { class: 'card-head' }, h('div', {}, h('h2', {}, f.name), h('div', { class: 'small muted' }, `${fmt.label(f.source_type)}${f.grant_number ? ' · ' + f.grant_number : ''} · ${fmt.date(f.fiscal_year_start)} – ${fmt.date(f.fiscal_year_end)}`)), can('budget:manage') ? h('div', { class: 'row' }, h('button', { class: 'btn sm', onClick: () => openLineForm(f, null, refresh) }, '+ Line'), h('button', { class: 'btn sm', onClick: () => openFundForm(f, refresh) }, 'Edit')) : null),
    ...
    h('div', { class: 'row between small muted' }, h('span', {}, `${f.pct_spent.toFixed(0)}% spent`, f.pct_spent > f.pct_elapsed + 15 ? ' — well ahead of the period' : f.pct_spent > f.pct_elapsed + 5 ? ' — ahead of the period' : ''), h('span', {}, `${f.pct_elapsed.toFixed(0)}% of period elapsed`)),
    h('div', { class: 'progress mb', 'aria-hidden': 'true' }, h('div', { class: f.pct_spent > f.pct_elapsed + 15 ? 'danger' : f.pct_spent > f.pct_elapsed + 5 ? 'warn' : '', style: { width: `${pct(f.spent, f.total_amount)}%` } })),
    ...
    f.lines.length ? table([
      ...
      { label: '%', render: l => { const p = pct(l.subtree_spent, l.allocated_amount); return h('div', { class: 'row nowrap', style: { gap: '.4rem' } }, h('div', { class: 'progress', style: { width: '80px' }, 'aria-hidden': 'true' }, h('div', { class: p > 90 ? 'danger' : '', style: { width: `${p}%` } })), flag(`${Math.round(p)}%`, p > 90, 'more than 90% spent')); } },
  ...
  return h('div', {},
    ...
    pageTabs([['overview', 'Funds'], ['expenditures', 'Expenditures'], ['analysis', 'Analysis']], tab, (k) => nav(`budget?tab=${k}`), { label: 'Funding & spending sections' }),
    ...
    tab === 'analysis' ? h('div', { class: 'grid cols-2' }, h('div', { class: 'card' }, h('h2', {}, 'Spend by category'), bars(sum.by_category, { valueKey: 'amount', labelKey: 'category', format: fmt.money })), h('div', { class: 'card' }, h('h2', {}, 'Spend by month'), bars(sum.by_month, { valueKey: 'amount', labelKey: 'month', format: fmt.money })), h('div', { class: 'card' }, h('h2', {}, 'Client assistance'), stat('Clients assisted', sum.per_client.clients), h('div', { class: 'mt' }, stat('Avg per client', fmt.money(sum.per_client.clients ? sum.per_client.amount / sum.per_client.clients : 0))))) : null);
});
