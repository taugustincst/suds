...
* **The CalOMS Tx "preview" was the real file, and the accounted file could differ from the one sent.**
  `GET /api/caloms/extract` returned every client's name and date of birth, unaccounted and with no
  mass-export check, byte-for-byte what DHCS would accept; "Mark as submitted" rebuilt the extract at click
  time, so records edited in between made the accounted set differ from what went out, and this document
  claimed the extract opened a mass-export incident when it did not. The preview now carries no names or
  dates of birth and is marked not for submission throughout; the submission file is produced once by
  `POST /api/caloms/submissions`, accounted against its id and SHA-256, kept encrypted, and served exactly by
  `GET /api/caloms/submissions/:id/file`; producing it and downloading it both run the mass-export check
  (docs/compliance/CALOMS.md, `test/caloms.test.js`).
* **FHIR ignored the consent's scope.** The API checked only the recipient and purpose, so a consent whose
  signed scope was "attendance only" still released overdose events, the risk level, note metadata,
  referrals and tasks. A consent now records the categories of information it covers as codes
  (`consents.info_categories`, ticked on the consent form; `CONSENT_INFO_CATEGORIES`), and the FHIR API —
  searches, reads, Bundles and `$export` — serves each resource type only for clients whose covering consent
  names its category. A consent with no categories (every one recorded earlier, unless its scope said plainly
  that it covers the whole record) covers nothing automated: a worker reading the free-text scope can still
  rely on it for a referral or a recorded disclosure; the machine cannot (docs/integration/FHIR.md,
  `test/fhir.test.js`).
* **The county EHR hand-off accepted "other".** A supervisor's written justification with basis "other"
  released every client's identified encounters with no per-client consent, although every other identified
  export refuses "other". The hand-off now takes only the identified-export bases (`EXPORT_BASES`, checked by
  `requireExportBasis`) and the State reporting page no longer offers "other" (`test/caloms.test.js`).
...
| **Automated and system disclosures: FHIR API, county EHR hand-off, CalOMS Tx** | All three write through `server/disclosure.js` `record()` with the same accounting fields (notice version, `legal_proceeding = 0`, `counseling_notes = 0`). **FHIR** (`fhirCoverage`, docs/integration/FHIR.md): only for clients whose consent `requireBasis` would accept without a further question — `part2_disclosure` naming the organisation for the purpose of use, or `part2_tpo` naming it (alone or in a class wording) — and, per resource type, only when that consent's coded categories cover it; a general ROI only when `part2_program` is off; never a counseling-notes or proceedings consent; clients with an agreed restriction are withheld. **County EHR hand-off** (`server/routes/handoff.js`): its own basis choice (each client's consent naming the recipient — the others are left out and listed by code — or, for the whole file, an identified-export basis checked by `requireExportBasis`: a registered QSOA, research or audit approval with the recipient, or `internal`; never "other", since a supervisor's justification is a one-client override, not a release of every client's encounters), consent types as for a consent-based export (`fileConsentTypes`), the agreed-restriction confirmation (`requireRestrictionReview`), never a proceeding. **CalOMS Tx** (`server/routes/caloms.js`): required by law, recorded under the fixed `state_reporting` basis — no basis to choose, no consent checked, not stoppable by an agreed restriction (§164.522(a)(1)(v)). The **preview** (`GET /api/caloms/extract`) cannot be submitted — files named `PREVIEW-…`, a *PREVIEW - NOT FOR SUBMISSION* README, `PREVIEW` / `NOT FOR SUBMISSION` in place of names and no dates of birth — so it is not an identified file: audited (`caloms.extract`, `preview: true`), nobody's accounting changes. **Produce submission file** (`POST /api/caloms/submissions`, `export:identified`) is the disclosure: the file is built once and kept encrypted with its SHA-256 (`caloms_submissions`), one accounting row per client in it (`source_ref` `caloms:<id>`, the hash in *what*), the records stamped as sent, audited as `caloms.submitted`, and a mass-export incident when it names many clients; `GET /api/caloms/submissions/:id/file` serves exactly those bytes (hash-checked, audited, mass-export check again), so the file sent is the file accounted. | Built |
...
* CalOMS Tx: check the **preview** (it has no names and cannot be submitted), then **Produce submission
  file** — that is when it is written to each client's accounting of disclosures — and send the file that
  downloads to DHCS unchanged.
