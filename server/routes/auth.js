...
const { hashPassword, verifyPassword, generateTotpSecret, verifyTotp, otpauthUrl, encrypt, decrypt } = require('../crypto');
...
module.exports = (r) => {
  ...
  r.post('/api/auth/password', (ctx) => {
    ...
    db.run(`UPDATE users SET password_hash=?, must_change_password=0, password_changed_at=?, updated_at=? WHERE id=?`, hashPassword(new_password), db.now(), db.now(), u.id);
