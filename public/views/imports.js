...
route('imports', async (r) => {
  ...
  const fileIn = h('input', { type: 'file', tabindex: '-1', 'aria-hidden': 'true', multiple: true, accept: '.json,.md,.txt,.mht,.mhtml,.html,.htm,.docx' });
  const drop = h('div', { class: 'dropzone', role: 'button', tabindex: '0', onClick: () => fileIn.click(), onKeydown: e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); fileIn.click(); } }, onDragover: e => { e.preventDefault(); drop.classList.add('over'); }, onDragleave: () => drop.classList.remove('over'), onDrop: async e => { e.preventDefault(); drop.classList.remove('over'); await uploadFiles(e.dataTransfer.files); } }, 'Drop export files here or click to choose', h('div', { class: 'small' }, state.local ? 'Files are parsed in your browser — nothing leaves this device — staged for your review, and only become notes after you assign a client.' : 'Files are parsed on the server, staged for your review, and only become notes after you assign a client.'));
...
async function importDetail(id) {
  ...
  const cards = staged.map(it => {
    ...
    return h('div', { class: 'card mb' },
      h('div', { class: 'card-head' }, h('div', {}, h('h2', {}, it.title || '(untitled)'), h('div', { class: 'small muted' }, it.captured_at ? `Captured ${fmt.dt(it.captured_at)}` : 'No date detected', it.external_id ? ` · ref ${it.external_id}` : '', it.metadata?.section ? ` · ${it.metadata.section}` : '', it.metadata?.tags?.length ? ` · ${it.metadata.tags.join(', ')}` : '')),
  ...
  return h('div', {},
    ...
    items.some(i => i.status !== 'staged') ? h('div', { class: 'card' }, h('h2', {}, 'Processed'), table([{ label: 'Title', key: 'title' }, { label: 'Captured', render: i => i.captured_at ? fmt.dt(i.captured_at) : '—' }, { label: 'Status', render: i => badge(fmt.label(i.status), statusKind(i.status)) }, { label: 'Note', render: i => i.note_id ? h('a', { href: '#', onClick: async (e) => { e.preventDefault(); (await import('./notes.js')).openNote(i.note_id, {}); } }, 'Open note') : '' }], items.filter(i => i.status !== 'staged'))) : null);
}
